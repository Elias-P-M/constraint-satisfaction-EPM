#!/usr/bin/env python3
"""
Active learning with EHVI × correlated POF (Gaussian copula):
...
- Selection uses acquisition = EHVI(...) * POF(all constraints incl. BCC),
  where POF uses a Gaussian copula for correlated constraints.
...
RESULTS_DIR = "results_ehvi_pofcorr"
PLOTS_DIR = "results_opt"
PLOT_PREFIX = "affine_progress_ehvipofcorr"
PRED_PLOT_PREFIX = "affine_pred_ehvipofcorr"
PLOTS_BASE_DIR = "plots_ehvi_pofcorr"
PLOT_AFFINE = False
PLOT_EVERY = 1
PARETO_BCC_MASK: Optional[np.ndarray] = None
COPULA_SHRINKAGE = 0.2
COPULA_MIN_POINTS = 5
POF_BATCH = 1000
...
def _estimate_copula_corr(
    models: Models,
    df_train_measured: pd.DataFrame,
    X_train_obj: np.ndarray,
) -> np.ndarray:
    if df_train_measured.shape[0] < COPULA_MIN_POINTS:
        return np.eye(4)
    means_m, sigmas_m, _, _ = gp_predict_all(models, X_train_obj, df_train_measured)
    mu_st = means_m[:, 0]
    mu_den = -(means_m[:, 1])
    mu_ys = means_m[:, 2]
    mu_pugh = means_m[:, 3]
    sd_st = np.maximum(sigmas_m[:, 0], EPS)
    sd_den = np.maximum(sigmas_m[:, 1], EPS)
    sd_ys = np.maximum(sigmas_m[:, 2], EPS)
    sd_pugh = np.maximum(sigmas_m[:, 3], EPS)

    y_st = df_train_measured["Solidus Temp"].to_numpy(float)
    y_den = df_train_measured["Density"].to_numpy(float)
    y_ys = df_train_measured["YS 600C"].to_numpy(float)
    y_pugh = df_train_measured["Pugh Ratio"].to_numpy(float)

    z = np.column_stack([
        (y_den - mu_den) / sd_den,
        (y_ys - mu_ys) / sd_ys,
        (y_pugh - mu_pugh) / sd_pugh,
        (y_st - mu_st) / sd_st,
    ])
    ...
    if not np.all(np.isfinite(R)):
        return np.eye(4)
    R = (1.0 - COPULA_SHRINKAGE) * R + COPULA_SHRINKAGE * np.eye(4)
    R = R + 1e-6 * np.eye(4)
    return R
...
def run_campaign(seed: int = 0, iterations: int = 100) -> None:
    ...
    for it in range(iterations):
        ...
        # --------- Predicted feasibility (Gaussian copula) ----------
        R = _estimate_copula_corr(models, df_train_measured, X_train_obj)
        signs = np.array([1.0, -1.0, -1.0, -1.0])
        thresh = np.column_stack([
            np.full_like(mu_den, DENSITY_THRESH),
            np.full_like(mu_ys, YS_THRESH),
            np.full_like(mu_pug, PUGH_THRESH),
            np.full_like(mu_st, ST_THRESH),
        ])
        mus = np.column_stack([mu_den, mu_ys, mu_pug, mu_st])
        sigs = np.column_stack([sd_den, sd_ys, sd_pug, sd_st])
        z = signs * (thresh - mus) / sigs
        D = np.diag(signs)
        R_adj = D @ R @ D

        mvn = multivariate_normal(mean=np.zeros(4), cov=R_adj, allow_singular=True)
        total_prob_no_bcc = np.zeros(len(df), dtype=float)
        for i in range(0, z.shape[0], POF_BATCH):
            total_prob_no_bcc[i:i + POF_BATCH] = mvn.cdf(z[i:i + POF_BATCH])
        total_prob_no_bcc = np.clip(total_prob_no_bcc, 0.0, 1.0)
...
def _configure_from_args(args: argparse.Namespace) -> tuple[list[int], int, int]:
    ...
    global RUN_NAME
    global COPULA_SHRINKAGE, COPULA_MIN_POINTS, POF_BATCH
    ...
    if args.copula_shrinkage is not None:
        COPULA_SHRINKAGE = args.copula_shrinkage
    if args.copula_min_points is not None:
        COPULA_MIN_POINTS = args.copula_min_points
    if args.pof_batch is not None:
        POF_BATCH = args.pof_batch
    if args.plot_affine:
        PLOT_AFFINE = True
    if args.plot_every:
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--copula-shrinkage", type=float, help="Shrinkage toward identity for copula corr")
    p.add_argument("--copula-min-points", type=int, help="Min measured points for copula corr")
    p.add_argument("--pof-batch", type=int, help="Batch size for MVN CDF evaluation")
